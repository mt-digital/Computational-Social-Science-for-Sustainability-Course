...
With the model defined, we can now run the model. We must first define how partner selection and interaction
work in functions called `partner_selection` and `interaction`. For this model there is no `model_step`. We then pass these functions to the `run` function, plus the desired number of time steps. Note that  *as arguments---we do not call the functions ourselves*.
[When we pass functions as arguments to other functions like this we are using the *functional programming* stye. Functions that take other functions as arguments are called *higher-order functions*. `run`, therefore, is a higher-order function.]{.aside}
...
interaction_success <- function(learner, teacher, model) {
