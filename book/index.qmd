...
## Developing models of social behavior for sustainability

These models share a general structure:
...
When considering the total change in $A,$ written $\Delta A,$ following a discrete time step, $\Delta t$, from time "now" to time "next" (\$t \to t+1\$), we can drop the indices since all indices are $t$, i.e.,
